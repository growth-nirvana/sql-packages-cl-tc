...
*/
{% assign dimensions = vars.google_ads_search.models.google_commercial_search__keyword_report.dimensions %}
{% assign metrics = vars.google_ads_search.models.google_commercial_search__keyword_report.metrics %}
{% assign account_id = vars.google_ads_search.account_ids %}
{% assign active = vars.google_ads_search.active %}
{% assign table_active = vars.google_ads_search.models.google_commercial_search__keyword_report.active %}
{% assign dataset_id = vars.output_dataset_id %}
{% assign table_id = vars.google_ads_search.models.google_commercial_search__keyword_report.table_id %}
...
{% assign source_table_id = 'keyword_report' %}
        ...
        */
        ...
        , report as(
            select
                ...
                , sum(safe_divide(cost_micros, 1000000)) as cost
                , sum(clicks) as clicks
                , sum(conversions) as conversions
                , sum(conversions_value) as conversions_value
                , avg(absolute_top_impression_percentage) as absolute_top_impression_percentage
                , avg(top_impression_percentage) as top_impression_percentage
                ...
                , avg(search_rank_lost_absolute_top_impression_share) as search_rank_lost_absolute_top_impression_share
                ...
                , avg(safe_cast(historical_quality_score as int64)) as historical_quality_score
            from
        ...
        */
        , api as(
            select
                ...
                , report.keyword_text
                , report.keyword_match_type
                , report.historical_creative_quality_score
                , report.historical_landing_page_quality_score
                , report.historical_search_predicted_ctr
                , report.impressions
                , report.cost
                , report.clicks
                , report.conversions
                , report.conversions_value
                , report.absolute_top_impression_percentage
                , report.top_impression_percentage
                , report.search_absolute_top_impression_share
                , report.search_budget_lost_top_impression_share
                , report.search_budget_lost_absolute_top_impression_share
                , report.search_impression_share
                , report.search_rank_lost_absolute_top_impression_share
                , report.search_rank_lost_top_impression_share
                , report.search_top_impression_share
                , report.historical_quality_score
                {% for conversion in conversions %}
                    /* 
                        We need to toggle these. For instance we may not always want all of these columns by
                        default because it may be confusing for the user. Perhaps we only typically want 
